<template>
	<view class="user">
		<view class="top">
			<view class="group" @click="gotoUserInfo">
				<view class="userinfo">
					<view class="pic">
	...
	methods: {
		...
		getnum() {
			...
			db.collection('quanzi_article')
				...
				.get()
				.then((res) => {
					this.belikednum = res.result.data[0].totalScore;
				});
			db.collection('quanzi_article')
				.where({
					delState: false,
				...
				.groupBy('user_id')
				.groupField('sum(comment_count) as totalScore ')
				.get()
				.then((res) => {
					this.comnum = res.result.data[0].totalScore;
				});
		}
	}
};
